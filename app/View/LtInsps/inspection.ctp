...
<div id="tools"> 
<p class="sort-by">Sort by: <a href="<?php echo Router::url('/', true).'LtInsps/inspection/date';?>" class="active" >Time</a> | <a href="<?php echo Router::url('/', true).'LtInsps/inspection/suburb';?>">Suburb</a></p>
</div>
...
?>
		<?php if(!isset($tmp_mth) || ($tmp_mth<>date('d M Y', strtotime($lt['LtInsp']['insp_start'])))){ ?>
			<li class="title"><h2><?php echo date('d M Y', strtotime($lt['LtInsp']['insp_start']));?></h2></li>
			<?php $tmp_mth=date('d M Y', strtotime($lt['LtInsp']['insp_start']));
		}?>
<li>
